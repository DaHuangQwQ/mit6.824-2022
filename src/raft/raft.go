...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	rf.persist()
	DPrintf("%d 收到了客户端 发来的请求 Term: %d index: %d", rf.me, term, index)
...
func (rf *Raft) Convert2Leader() {
	...
	for i := 0; i < len(rf.log); i++ {
		rf.log[i].Index = rf.log[len(rf.log)-1].Index + 1
	}
...
func (rf *Raft) LeaderSendEntries() {
	...
	for i := range rf.peers {
		...
		go func(i int) {
			...
			preLog := rf.log[rf.nextIndex[i]] // preLog是leader要发给server idx的日志条目的前一个日志条目
			args := EntriesArgs{
				Term:         rf.currentTerm,
				LeaderId:     rf.me,
...
func (rf *Raft) AppendEntries(args *EntriesArgs, reply *EntriesApply) {
	...
	DPrintf("%d 向 %d 发来 log  PrevLogIndex: %d  PrevLogTerm: %d", args.LeaderId, rf.me, args.PrevLogIndex, args.PrevLogTerm)
	rf.timer.Stop()
	rf.timer.Reset(GetRandTimer())
	// DPrintf("%d 的定时器 重新 设置", rf.me)
	rf.Status = Follower
